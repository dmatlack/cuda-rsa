...
Integer factor(Integer n, Integer a, Integer K) {
  ...
  for (;;iteration++) {
    count ++;
    if (gcd(a, n) > 1) {
      // cout << "Ran " << count << " iterations." << endl;
  ...
  // cout << "Ran " << count << " iterations (and failed)." << endl;
...
int main(int argc, char *argv[]) {
  ...
  while(!isprime(n)) {
    ...
    cout << pf << " (in " << elapsed_us << " us)" << flush;
    n /= pf;
  }
  cout << n << endl;
  return 0;
